...
export const getProducts = async (filters?: { category?: string; featured?: boolean; limit?: number }) => {
  const path='products';
  try {
    let q=query(collection(db,path)); if(filters?.category) q=query(q,where('category','==',filters.category)); if(filters?.featured) q=query(q,where('featured','==',true)); if(filters?.limit) q=query(q,limit(filters.limit));
  ...
  } catch (error) { handleFirestoreError(error, OperationType.LIST, path); }
};
...
export const getProductBySlug = async (slug: string) => {
  const path='products';
  try {
    const slugQuery=query(collection(db,path),where('slug','==',slug),limit(1)); let snapshot=await getDocs(slugQuery);
    if(!snapshot.empty) return normalizeProduct(snapshot.docs[0].data() as Record<string,any>,snapshot.docs[0].id);
    const handleQuery=query(collection(db,path),where('handle','==',slug),limit(1)); snapshot=await getDocs(handleQuery); if(snapshot.empty) return null;
...
export const getProductById = async (id: string) => {
  ...
  try { const productSnap=await getDoc(doc(db,'products',id)); if(!productSnap.exists()) return null; return normalizeProduct(productSnap.data() as Record<string,any>,productSnap.id); }
  catch (error) { handleFirestoreError(error, OperationType.GET, path); }
};

type ProductPayload = Omit<Product, 'id' | 'createdAt' | 'updatedAt'>;
export const createProduct = async (product: ProductPayload) => { const path='products'; try { return await addDoc(collection(db,path),{...product,createdAt:serverTimestamp(),updatedAt:serverTimestamp()}); } catch (error) { handleFirestoreError(error, OperationType.CREATE, path);} };
export const updateProduct = async (id: string, data: Partial<ProductPayload>) => { const path=`products/${id}`; try { await updateDoc(doc(db,'products',id),{...data,updatedAt:serverTimestamp()}); } catch (error) { handleFirestoreError(error, OperationType.UPDATE, path);} };
export const deleteProduct = async (id: string) => { const path=`products/${id}`; try { await deleteDoc(doc(db,'products',id)); } catch (error) { handleFirestoreError(error, OperationType.DELETE, path);} };
...
export const deleteAllProducts = async (onProgress?: (info: { completed: number; total: number; failed: number }) => void) => {
  ...
  return deleteProductsByIds(ids, onProgress);
};

export const toggleProductActive = async (id: string, active: boolean) => updateProduct(id, { active });
