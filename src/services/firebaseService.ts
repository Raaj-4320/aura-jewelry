...
export const createProduct = async (product: ProductPayload) => { const path='products'; const startedAt=Date.now(); logDB('createProduct_request_start',{name:(product as any).name,slug:(product as any).slug}); await logAdminWriteAuthContext('createProduct'); try { const ref=await addDoc(collection(db,path),{...product,createdAt:serverTimestamp(),updatedAt:serverTimestamp()}); logDB('createProduct_request_success',{id:ref.id,durationMs:Date.now()-startedAt}); return ref; } catch (error) { logError('createProduct_request_failure', error,{name:(product as any).name}); handleFirestoreError(error, OperationType.CREATE, path);} };
export const updateProduct = async (id: string, data: Partial<ProductPayload>) => { const path=`products/${id}`; const startedAt=Date.now(); logDB('updateProduct_request_start',{id,keys:Object.keys(data||{})}); await logAdminWriteAuthContext('updateProduct'); try { await updateDoc(doc(db,'products',id),{...data,updatedAt:serverTimestamp()}); logDB('updateProduct_request_success',{id,durationMs:Date.now()-startedAt}); } catch (error) { logError('updateProduct_request_failure', error,{id}); handleFirestoreError(error, OperationType.UPDATE, path);} };
