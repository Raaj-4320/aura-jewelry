...
function sanitizeForFirestoreInternal(input: unknown, path = ''): SanitizedResult {
  ...
  if (typeof input === 'object') {
    ...
    if (prototype !== Object.prototype && prototype !== null) return { value: input, removedPaths: [] };
...
function normalizeProduct(raw: Record<string, any>, id: string): Product {
  ...
  const rawQuantity = raw.quantity ?? raw.inventoryQty ?? raw.inventory?.qty;
  const quantity = Number.isFinite(Number(rawQuantity)) ? Math.max(0, Number(rawQuantity)) : (raw.active === false ? 0 : 1);

  return {
    id, name, slug, productCode: raw.productCode || raw.code || raw.sku || '', category, subcategory,
    ...
    price, quantity, priceOnRequest: raw.priceOnRequest ?? price <= 0, currency: raw.currency || 'INR',
