...
export function getAutomaticShopLink(collection: HomepageCollection, categorySlug = '') {
  if (collection === 'new-arrivals') return '/shop?sort=newest';
  if (collection === 'bridal') return '/shop?category=bridal-sets';
  if (collection === 'minimal') return '/shop?subcategory=minimal';
  if (collection === 'category' && categorySlug) return `/shop?category=${categorySlug}`;
  return '/shop';
}

export function createEmptyHomepageSection(): HomepageSection {
  return { id: id('section'), visible: true, title: 'Collection', subtitle: 'Curated Selection', collection: 'featured', categorySlug: '', productLimit: 4, shopLink: getAutomaticShopLink('featured') };
}
...
function cleanSections(value: unknown): HomepageSection[] {
  ...
  const sections = value.flatMap((entry, index) => {
    ...
    return [{ id: sectionId, visible: raw.visible !== false, title: text(raw.title, 'Collection'), subtitle: text(raw.subtitle, 'Curated Selection'), collection, categorySlug: text(raw.categorySlug), productLimit: boundedInt(raw.productLimit, 4, 1, 12), shopLink: getAutomaticShopLink(collection, text(raw.categorySlug)) }];
  });
