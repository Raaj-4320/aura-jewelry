...
const DEFAULT_ADMIN_AUTH_CONTINUE_URL = 'https://sviwacreation.com/admin-login';

function getDefaultAdminAuthContinueUrl() {
  return ['localhost', '127.0.0.1'].includes(window.location.hostname)
    ? `${window.location.origin}${ADMIN_LOGIN_PATH}`
    : DEFAULT_ADMIN_AUTH_CONTINUE_URL;
}

function getAdminAuthContinueUrl() {
  const configuredUrl = import.meta.env.VITE_ADMIN_AUTH_CONTINUE_URL?.trim();
  if (!configuredUrl) return getDefaultAdminAuthContinueUrl();
  try {
    const url = new URL(ADMIN_LOGIN_PATH, configuredUrl);
    if (!['https:', 'http:'].includes(url.protocol)) throw new Error('Unsupported protocol');
    return url.toString();
  } catch {
    logAuth('admin_email_link_continue_url_invalid', { configured: true });
    return getDefaultAdminAuthContinueUrl();
  }
}

function getContinueUrlHost(continueUrl?: string) {
  try { return continueUrl ? new URL(continueUrl).host : window.location.host; } catch { return window.location.host; }
}
...
function getFriendlyAuthMessage(code?: string, fallback = 'Unable to complete sign-in. Please try again.', continueUrl?: string) {
  switch (code) {
    ...
    case 'auth/unauthorized-continue-uri': return `Firebase rejected the login-link return domain (${getContinueUrlHost(continueUrl)}). Add it in Firebase Authentication → Settings → Authorized domains, or set VITE_ADMIN_AUTH_CONTINUE_URL to an authorized stable URL and redeploy.`;
...
export default function AdminLogin() {
  ...
  const emailLinkContinueUrl = useMemo(() => getAdminAuthContinueUrl(), []);
  const emailLinkContinueHost = useMemo(() => getContinueUrlHost(emailLinkContinueUrl), [emailLinkContinueUrl]);
  ...
  const handleSendLink = async () => {
    setLoading(true); logAuth('admin_email_link_send_start', { email, continueHost: emailLinkContinueHost });
    try {
      await sendSignInLinkToEmail(auth, email, { url: emailLinkContinueUrl, handleCodeInApp: true });
    ...
    } catch (error) { const firebaseError = error as { code?: string; message?: string }; logError('admin_email_link_send_failure', error, { code: firebaseError.code }); toast.error(getFriendlyAuthMessage(firebaseError.code, firebaseError.message, emailLinkContinueUrl)); }
    finally { setLoading(false); }
  };
