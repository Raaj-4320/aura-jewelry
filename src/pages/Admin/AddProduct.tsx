          ...
          <div className="space-y-4 lg:col-span-7">
            <section className="space-y-3 rounded-2xl border border-rose-gold/15 bg-white p-4 shadow-sm">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="space-y-1"><span className="text-[10px] font-semibold uppercase tracking-widest text-taupe">Category</span><select name="category" value={formData.category} onChange={handleInputChange} className="w-full rounded-xl bg-warm-gray/40 px-4 py-3 text-sm">{categories.map((c) => <option key={c.id} value={c.slug}>{c.name}</option>)}</select></label>
                <label className="space-y-1"><span className="text-[10px] font-semibold uppercase tracking-widest text-taupe">Subcategory</span><select name="subcategory" value={formData.subcategory} onChange={handleInputChange} className="w-full rounded-xl bg-warm-gray/40 px-4 py-3 text-sm">{subcategories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
