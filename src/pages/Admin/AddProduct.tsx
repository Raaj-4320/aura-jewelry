      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white p-8 rounded-[2.5rem] border border-rose-gold/10 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <select name="subcategory" value={formData.subcategory} onChange={handleInputChange} className="w-full px-6 py-4 bg-warm-gray/30 rounded-2xl text-sm">{subcategories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
              </div>
              <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-8">
            ...
            <button type="submit" disabled={isSaving || uploading} className="w-full btn-primary py-5 flex items-center justify-center gap-3 disabled:opacity-50">
              {(isSaving || uploading) ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />}
              {uploading ? 'Uploading images…' : isSaving ? 'Saving…' : (isEdit ? 'Update Product' : 'Create Product')}
            </button>
            {uploading && <p className="text-xs text-amber-700">Please wait for image uploads to finish.</p>}
          </div>
        </form>
      </div>
