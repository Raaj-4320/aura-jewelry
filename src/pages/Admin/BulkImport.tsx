...
import { deleteAllProducts, getAdminProducts, sanitizeForFirestore } from '../../services/firebaseService';
    ...
    for (let i = 0; i < analysis.ready.length; i++) {
      ...
      try {
        ...
        if (exists.exists()) { skipped += 1; }
          ...
          else { const { sanitized } = sanitizeForFirestore(item.product as Record<string, unknown>); await setDoc(ref, sanitized as any, { merge: false }); created += 1; }
        }
      } catch { failed += 1; }
      const completed = i + 1;
