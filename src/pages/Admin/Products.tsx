...
import { logDB, logError, logProduct, logRoute, logUI } from '../../utils/logger';
...
export default function Products() {
  ...
  const fetchProducts = async () => {
    ...
    try { logDB('getAdminProducts_request_start'); const data = await getAdminProducts(); setProducts(data || []); logDB('getAdminProducts_request_success', { count: (data || []).length }); }
  ...
  useEffect(() => { logProduct('products_table_rendered', { total: products.length, visible: filteredProducts.length, active: products.filter(p=>p.active).length, inactive: products.filter(p=>!p.active).length, columns: ['checkbox','image','product','category/type','price','status','actions'] }); }, [products, filteredProducts.length]);
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-grow"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-taupe/40" size={18} /><input type="text" placeholder="Search by name or category..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); logUI('products_search_changed', { query: e.target.value }); }} className="w-full pl-12 pr-4 py-3 bg-white border border-rose-gold/10 rounded-2xl text-sm" /></div>
