      ...
      <div className="space-y-6">
        ...
        <section className="bg-white rounded-[2rem] border border-rose-gold/20 shadow-sm p-5 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-3">
            <select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)} className="rounded-xl border border-rose-gold/20 bg-ivory px-3 py-2 text-xs text-deep-taupe"><option value="all">All categories</option>{categories.map((category) => <option key={category.id} value={category.slug}>{category.name}</option>)}</select>
            <select value={subcategoryFilter} onChange={(event) => setSubcategoryFilter(event.target.value)} className="rounded-xl border border-rose-gold/20 bg-ivory px-3 py-2 text-xs text-deep-taupe"><option value="all">All subcategories</option>{subcategories.map((subcategory) => <option key={subcategory.id} value={subcategory.id}>{subcategory.name}</option>)}</select>
            <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="rounded-xl border border-rose-gold/20 bg-ivory px-3 py-2 text-xs text-deep-taupe"><option value="all">All statuses</option><option value="in-stock">In stock</option><option value="stockout">Stockout</option></select>
            ...
            <select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="rounded-xl border border-rose-gold/20 bg-ivory px-3 py-2 text-xs text-deep-taupe"><option value="name-asc">Name A–Z</option><option value="name-desc">Name Z–A</option><option value="price-asc">Price low–high</option><option value="price-desc">Price high–low</option><option value="newest">Newest first</option></select>
                ...
                {loading ? [1, 2, 3].map((i) => <tr key={i}><td colSpan={8} className="px-6 py-8 animate-pulse" /></tr>) : filteredProducts.map((product: any) => {
                  ...
                  return (
                    <tr key={product.id} className="hover:bg-warm-gray/10 align-top">
                      ...
                      <td className="px-4 py-3"><div className="grid gap-2"><select value={draft.category} onChange={(event) => updateRowDraft(product, 'category', event.target.value)} className="min-w-32 rounded-lg border border-warm-gray bg-white px-2 py-1 text-xs text-taupe uppercase">{draft.category && !categories.some((category) => category.slug === draft.category) && <option value={draft.category}>{draft.category}</option>}{categories.map((category) => <option key={category.id} value={category.slug}>{category.name}</option>)}</select><select value={draft.subcategory} onChange={(event) => updateRowDraft(product, 'subcategory', event.target.value)} className="min-w-32 rounded-lg border border-warm-gray bg-white px-2 py-1 text-xs text-taupe uppercase"><option value="">No subcategory</option>{draft.subcategory && !subcategories.some((subcategory) => subcategory.id === draft.subcategory) && <option value={draft.subcategory}>{draft.subcategory}</option>}{subcategories.map((subcategory) => <option key={subcategory.id} value={subcategory.id}>{subcategory.name}</option>)}</select></div></td>
