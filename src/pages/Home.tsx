...
function ProductSection({ title, subtitle, products, shopLink }: { title: string; subtitle: string; products: Product[]; shopLink: string }) {
  return (
    <section className="bg-blush py-16 sm:py-20 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-10 sm:mb-12">
          ...
          <Link to={shopLink} className="text-sm font-medium text-taupe border-b border-rose-gold/30 hover:border-rose-gold transition-all pb-1">
            View All Products
          </Link>
        </div>
...
export default function Home() {
  ...
  const heroImage = useMemo(
    () => sections.featured[0]?.thumbnailImage || sections.newArrivals[0]?.thumbnailImage || '',
    [sections]
  );
  ...
  const hasMinimalSubcategory = useMemo(
    () => allProducts.some((p) => normalizeSubcategory(p.subcategory) === 'minimal'),
    [allProducts]
  );
    ...
    <div className="space-y-16 sm:space-y-20 pb-20">
      <section className="relative h-[80vh] sm:h-[90vh] flex items-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          {heroImage ? (
            <img
              src={heroImage}
              alt="Sviwa Creation jewelry"
              className="w-full h-full object-cover opacity-60"
              referrerPolicy="no-referrer"
              onError={(e) => { e.currentTarget.src = JEWELRY_IMAGE_FALLBACK; }}
            />
          ) : (
            <div className="w-full h-full bg-gradient-to-br from-blush via-ivory to-champagne" />
          )}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="max-w-xl space-y-8">
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              ...
              <Link to={hasBridalCategory ? '/shop?category=bridal-sets' : '/shop'} className="btn-secondary">Bridal Edit</Link>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          {categoryCards.map((cat, idx) => (
            <motion.div key={cat.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} viewport={{ once: true }} className="group relative aspect-square overflow-hidden rounded-3xl bg-warm-gray">
              <Link to={cat.hasProducts ? `/shop?category=${cat.slug}` : '/shop'}>
      ...
      {loading ? (
        ...
        <>
          ...
          <ProductSection title="Featured Collection" subtitle="Curated Selection" products={sections.featured} shopLink="/shop" />
          <ProductSection title="New Arrivals" subtitle="Just Added" products={sections.newArrivals} shopLink="/shop?sort=newest" />
          <ProductSection
            title="Bridal Edit"
            subtitle="Wedding Spotlight"
            products={sections.bridal}
            shopLink={hasBridalCategory ? '/shop?category=bridal-sets' : '/shop'}
          />
          <ProductSection
            title="Minimal Favorites"
            subtitle="Everyday Elegance"
            products={sections.minimal}
            shopLink={hasMinimalSubcategory ? '/shop?subcategory=minimal' : '/shop'}
          />
        </>
      )}
