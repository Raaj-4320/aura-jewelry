...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      ...
      setLoading(true);
      try {
        logDB('product_detail_fetch_start', { slug });
        const data = await getProductBySlug(slug);
        if (data) {
          logDB('product_detail_fetch_success', { id: data.id, name: data.name, slug: data.slug, imageCount: (data.galleryImages||[]).length, active: data.active });
          setProduct(data);
  ...
  const productUrl = window.location.href || fallbackProductUrl;
  const primaryImageUrl = product.mainImage || product.thumbnailImage || '';
  const whatsappMessageLines = [
    "Hello, I'm interested in this jewelry item:",
    '',
    `Product: ${product.name}`,
    ...(productUrl ? [`Product URL: ${productUrl}`] : []),
    ...(primaryImageUrl ? [`Product Image: ${primaryImageUrl}`] : []),
    '',
