...
5. In Vercel Project Settings → **Environment Variables**, set this for Production and Preview, then redeploy:
   - `VITE_ADMIN_AUTH_CONTINUE_URL=https://sviwacreation.com/admin-login`
6. Optional: add an individual Vercel preview deployment domain to Firebase Authorized domains only if you intentionally want email-link bootstrap to return directly to that preview instead of the stable callback.
7. Optional but recommended: configure a Firebase Authentication password policy and email-enumeration protection.
8. Optional: customize the **Password reset** email template under Authentication → Templates.
...
- Email-link first-login bootstrap uses `VITE_ADMIN_AUTH_CONTINUE_URL` when configured. Set it to the stable authorized production callback `https://sviwacreation.com/admin-login` for Vercel Production and Preview.
- If `VITE_ADMIN_AUTH_CONTINUE_URL` is accidentally omitted on a deployed host, the app now defaults to `https://sviwacreation.com/admin-login` instead of a changing Vercel commit URL. Localhost development still uses its local origin so local testing remains possible when `localhost` is explicitly authorized in Firebase.
...
### Troubleshooting `auth/unauthorized-continue-uri`

If **Send first-time login link** fails with `auth/unauthorized-continue-uri`, Firebase rejected the email link's return domain. This is expected when a Vercel commit-preview URL is used as the callback without being allowlisted.

Required fix:
1. Firebase Console → Authentication → Settings → Authorized domains → add `sviwacreation.com`.
2. Redeploy Vercel so the app uses the stable production callback fallback.
3. Retry **Send first-time login link**. The login page should display `sviwacreation.com` as its callback host.

Recommended explicit Vercel configuration:
1. Vercel Project Settings → Environment Variables → add `VITE_ADMIN_AUTH_CONTINUE_URL=https://sviwacreation.com/admin-login` for Production and Preview.
2. Redeploy Vercel. This keeps the callback explicit and allows future environments to override it safely.

Alternative: add the individual preview deployment domain to Firebase Authorized domains. This is less convenient for commit URLs because the domain changes between previews.
