...
- Email-link first-login bootstrap uses `VITE_ADMIN_AUTH_CONTINUE_URL` when configured. Set it to the stable authorized production callback `https://sviwacreation.com/admin-login` for Vercel Production and Preview so arbitrary commit-preview domains do not need to be added one by one.
- If `VITE_ADMIN_AUTH_CONTINUE_URL` is unset, the app falls back to the current deployment origin. Firebase then requires that exact preview domain in **Authorized domains**.
...
2. Vercel Project Settings → Environment Variables → add `VITE_ADMIN_AUTH_CONTINUE_URL=https://sviwacreation.com/admin-login` for Production and Preview.
3. Redeploy Vercel.
4. Retry **Send first-time login link**. The login page displays the callback host it will use.
